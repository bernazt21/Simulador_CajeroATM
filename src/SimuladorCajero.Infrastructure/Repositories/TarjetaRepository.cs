...
public sealed class TarjetaRepository : ITarjetaRepository
{
    ...
    public TarjetaRepository(SqlConnectionFactory connectionFactory)
    {
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        command.Parameters.Add(
            new SqlParameter("@NumeroTarjeta", SqlDbType.Char, 16)
            {
                Value = numeroTarjeta
            });
        ...
        tarjeta.EstablecerNipHash(
            reader.GetString(reader.GetOrdinal("NipHash")));

        var intentosFallidos = reader.GetByte(
            reader.GetOrdinal("IntentosFallidos"));

        for (var intento = 0; intento < intentosFallidos; intento++)
        {
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        command.Parameters.Add(
            new SqlParameter("@IdTarjeta", SqlDbType.Int)
            {
                Value = idTarjeta
            });

        command.Parameters.Add(
            new SqlParameter("@NuevoNipHash", SqlDbType.NVarChar, 255)
            {
                Value = nuevoNipHash
            });
        ...
        CancellationToken cancellationToken = default)
    {
        ...
        command.Parameters.Add(
            new SqlParameter("@Bloqueada", SqlDbType.Bit)
            {
                Value = tarjeta.Bloqueada
            });

        command.Parameters.Add(
            new SqlParameter("@IntentosFallidos", SqlDbType.TinyInt)
            {
                Value = tarjeta.IntentosFallidos
            });

        command.Parameters.Add(
            new SqlParameter("@Activa", SqlDbType.Bit)
            {
